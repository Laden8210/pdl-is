        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="text-muted-foreground">Added By:</span> {pdl.personnel ? `${pdl.personnel.fname} ${pdl.personnel.lname}` : '-'}
              </div>
              <div className="col-span-2">
                <span className="text-muted-foreground">Address:</span> {`${pdl.brgy || ''}, ${pdl.city || ''}, ${pdl.province || ''}`}
              </div>
            </div>
          </div>
          ...
          <Accordion type="multiple" className="w-full">
            ...
            {Array.isArray(pdl.court_orders) && pdl.court_orders.length > 0 && (
              <AccordionItem value="court">
                <AccordionTrigger className="text-sm font-medium">
                  Court Orders ({pdl.court_orders.length})
                </AccordionTrigger>
                <AccordionContent className="space-y-4 p-2">
                  {pdl.court_orders.map((order, index) => (
                    <div key={index} className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      <div>
                        <span className="text-muted-foreground">Order Date:</span> {format(new Date(order.order_date), 'MMM dd, yyyy')}
                      </div>
                      <div>
                        <span className="text-muted-foreground">Received Date:</span> {format(new Date(order.received_date), 'MMM dd, yyyy')}
                      </div>
                      <div>
            ...
            {Array.isArray(pdl.medical_records) && pdl.medical_records.length > 0 && (
              <AccordionItem value="medical">
                <AccordionTrigger className="text-sm font-medium">
                  Medical Records ({pdl.medical_records.length})
                </AccordionTrigger>
                <AccordionContent className="space-y-4 p-2">
                ...
                <AccordionContent className="space-y-4 p-2">
                  {pdl.cases.map((caseInfo, index) => (
                    <div key={index} className="grid grid-cols-2 gap-2 text-sm border rounded-lg p-3">
                      ...
                      <div>
                        <span className="text-muted-foreground">Date Committed:</span> {format(new Date(caseInfo.date_committed), 'MMM dd, yyyy')}
                      </div>
                      <div>
