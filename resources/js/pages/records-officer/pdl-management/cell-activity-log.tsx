...
export default function CellActivityLog() {
    ...
    const { transferLogs, cell, filters, pagination, auth } = props;
    ...
    const transferLogData = transferLogs.map((log) => ({
        ...
        transferred_by: log.transferred_by ? `${log.transferred_by.fname} ${log.transferred_by.lname}` : 'Unknown User', // Fallback for missing user
        reason: log.reason || 'No reason provided',
        transferred_at: log.transferred_at,
    }));

    const handleSearch = () => {
        const url = cell ? `/admin/pdl-management/cell-activity-log?cell_id=${cell.cell_id}` : '/admin/pdl-management/cell-activity-log';
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-6 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">{cell ? `Cell Transfer Activity Log - ${cell.cell_name}` : 'Cell Transfer Activity Log'}</h1>
                    <div className="flex items-center space-x-2">
                        {cell && (
                            <Button variant="outline" onClick={() => router.get('/admin/cell-management')}>
                                Back to Cell Management
                            </Button>
                        )}
                        {role === 'admin' && (
                            <Button variant="outline" onClick={() => router.get('/admin/pdl-management/cell-assignment')}>
                                Back to Assignments
                            </Button>
                        )}
                        {role === 'record-officer' && (
                            <Button variant="outline" onClick={() => router.get('/record-officer/pdl-management/cell-assignment')}>
                                Back to Assignments
                            </Button>
                        )}
                    </div>
